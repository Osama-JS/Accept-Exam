...
@section('title', 'تعديل اختبار: ' . $exam->title)
@section('page-title', 'تعديل اختبار: ' . $exam->title)

@section('breadcrumb')
    <a href="{{ route('admin.exams.index') }}">إدارة الاختبارات</a>
    <span>/</span>
    <span style="color: var(--text-main); font-weight: 700;">تعديل اختبار</span>
@endsection
...
@if($errors->any())
    <div class="alert alert-danger" style="border-radius: 12px; margin-bottom: 24px; padding: 16px; font-size: 13px; font-weight: 700;">
...
<form method="POST" action="{{ route('admin.exams.update', $exam) }}" id="exam-form">
    @csrf
    @method('PUT')
    ...
    <div class="workspace-grid">
        ...
        <div class="ws-card">
            <div class="ws-card-header">
                <h3 class="ws-card-title"><i class="bi bi-pencil-square text-primary" style="font-size: 20px;"></i> تعديل إعدادات الاختبار الأساسية</h3>
            ...
            <div class="ws-card-body">
                        ...
                        <div class="input-icon-group">
                            ...
                            <select name="academic_year_id" class="form-control" required>
                                @foreach($academicYears as $year)
                                    <option value="{{ $year->id }}" {{ $year->id == old('academic_year_id', $exam->academic_year_id) ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-icon-group">
                            ...
                            <select name="grade_id" id="target-grade" class="form-control" onchange="validateGrades(); updateAllStats();" required>
                                ...
                                @foreach($grades as $g)
                                    <option value="{{ $g->id }}" {{ $g->id == old('grade_id', $exam->grade_id) ? 'selected' : '' }}>{{ $g->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="form-group mt-3">
                    ...
                    <div class="input-icon-group">
                        ...
                        <input type="text" name="title" class="form-control" value="{{ old('title', $exam->title) }}" placeholder="مثال: اختبار القبول للصف الأول المتوسط 2026" required>
                    </div>
                </div>
                ...
                <div class="grid-2 mt-3">
                    <div class="form-group">
                        ...
                        <div class="input-icon-group">
                            ...
                            <input type="number" name="duration_minutes" class="form-control" value="{{ old('duration_minutes', $exam->duration_minutes) }}" min="10" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-icon-group">
                            ...
                            @php
                                $currentPassPercent = $exam->total_marks > 0 ? (int)round(($exam->pass_marks / $exam->total_marks) * 100) : 50;
                            @endphp
                            <input type="number" name="pass_marks_percent" class="form-control" value="{{ old('pass_marks_percent', $currentPassPercent) }}" min="1" max="100" required>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="ws-card">
            ...
            <div class="ws-card-body">
                <div id="configs-list">
                    @php
                        $configs = old('configs') ? collect(old('configs'))->map(function($item) {
                            return (object)[
                                'grade_id' => $item['grade_id'] ?? null,
                                'subject_id' => $item['subject_id'] ?? null,
                                'question_count' => $item['question_count'] ?? 0,
                                'marks_per_question' => $item['marks_per_question'] ?? 1,
                                'sort_order' => $item['sort_order'] ?? 1,
                                'difficulties' => [
                                    'easy' => ['count' => $item['easy_count'] ?? 0, 'marks' => $item['easy_marks'] ?? 1],
                                    'medium' => ['count' => $item['medium_count'] ?? 0, 'marks' => $item['medium_marks'] ?? 1],
                                    'hard' => ['count' => $item['hard_count'] ?? 0, 'marks' => $item['hard_marks'] ?? 1],
                                ],
                                'types' => [
                                    'mcq' => $item['mcq_count'] ?? 0,
                                    'tf' => $item['tf_count'] ?? 0,
                                    'matching' => $item['matching_count'] ?? 0,
                                    'essay' => $item['essay_count'] ?? 0,
                                ],
                                'grade' => \App\Models\Grade::find($item['grade_id'] ?? null),
                            ];
                        }) : $exam->subjectConfigs;
                    @endphp
                    ...
                    @foreach($configs as $index => $config)
                        @php
                            ...
                            $configGradeSubjects = $configGrade ? $configGrade->subjects : collect();
                            $diffs = $config->difficulties ?? [];
                            $easyVal = isset($diffs['easy']) && is_array($diffs['easy']) ? (int)($diffs['easy']['count'] ?? 0) : (int)($diffs['easy'] ?? 0);
                            $easyMarks = isset($diffs['easy']) && is_array($diffs['easy']) ? (int)($diffs['easy']['marks'] ?? 1) : 1;
                            $mediumVal = isset($diffs['medium']) && is_array($diffs['medium']) ? (int)($diffs['medium']['count'] ?? 0) : (int)($diffs['medium'] ?? 0);
                            $mediumMarks = isset($diffs['medium']) && is_array($diffs['medium']) ? (int)($diffs['medium']['marks'] ?? 1) : 1;
                            $hardVal = isset($diffs['hard']) && is_array($diffs['hard']) ? (int)($diffs['hard']['count'] ?? 0) : (int)($diffs['hard'] ?? 0);
                            $hardMarks = isset($diffs['hard']) && is_array($diffs['hard']) ? (int)($diffs['hard']['marks'] ?? 1) : 1;

                            $types = $config->types ?? [];
                            $mcqVal = $types['mcq'] ?? 0;
                            $tfVal = $types['tf'] ?? 0;
                            $matchingVal = $types['matching'] ?? 0;
                            $essayVal = $types['essay'] ?? 0;
                        @endphp
                        
                        <div class="source-card">
                            <div class="source-card-header">
                                <span class="source-card-title"><i class="bi bi-book"></i> المادة المصدر #{{ $index + 1 }}</span>
                                @if($index > 0)
                            ...
                            <div class="source-inputs">
                                <div>
                                    ...
                                    <div class="input-icon-group">
                                        ...
                                        <select name="configs[{{ $index }}][grade_id]" class="form-control grade-config-select" onchange="loadConfigSubjects(this, {{ $index }})">
                                            <option value="">-- الصف --</option>
                                            @foreach($grades as $g)
                                                <option value="{{ $g->id }}" {{ $configGrade && $configGrade->id == $g->id ? 'selected' : '' }}>{{ $g->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="input-icon-group">
                                        ...
                                        <select name="configs[{{ $index }}][subject_id]" class="form-control subject-config-select" onchange="validateGrades(); loadSubjectStats(this);" required>
                                            <option value="">-- اختر الصف أولاً --</option>
                                            @foreach($configGradeSubjects as $sub)
                                                <option value="{{ $sub->id }}" {{ $config->subject_id == $sub->id ? 'selected' : '' }}>{{ $sub->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="input-icon-group">
                                        ...
                                        <input type="number" name="configs[{{ $index }}][question_count]" class="form-control question-count-input" value="{{ $config->question_count }}" min="1" oninput="validateConfigStats(this)" required>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="input-icon-group">
                                        ...
                                        <input type="number" name="configs[{{ $index }}][marks_per_question]" class="form-control" value="{{ $config->marks_per_question }}" min="1" required>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="input-icon-group">
                                        ...
                                        <input type="number" name="configs[{{ $index }}][sort_order]" class="form-control" value="{{ $config->sort_order }}" min="1" required>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="config-inputs-row" style="display: flex; flex-direction: column; gap: 8px;">
                                            ...
                                            <div style="display: flex; align-items: center; justify-content: space-between; background: #f8fafc; padding: 6px 10px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                                ...
                                                <div style="display: flex; gap: 6px; align-items: center;">
                                                    <input type="number" name="configs[{{ $index }}][easy_count]" class="form-control" min="0" value="{{ $easyVal ?: '' }}" placeholder="العدد" style="width: 70px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                                    <span style="font-size: 10px; color: #94a3b8;">درجة:</span>
                                                    <input type="number" name="configs[{{ $index }}][easy_marks]" class="form-control" min="1" value="{{ $easyMarks }}" placeholder="الدرجة" style="width: 50px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                                </div>
                                            ...
                                            <div style="display: flex; align-items: center; justify-content: space-between; background: #f8fafc; padding: 6px 10px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                                ...
                                                <div style="display: flex; gap: 6px; align-items: center;">
                                                    <input type="number" name="configs[{{ $index }}][medium_count]" class="form-control" min="0" value="{{ $mediumVal ?: '' }}" placeholder="العدد" style="width: 70px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                                    <span style="font-size: 10px; color: #94a3b8;">درجة:</span>
                                                    <input type="number" name="configs[{{ $index }}][medium_marks]" class="form-control" min="1" value="{{ $mediumMarks }}" placeholder="الدرجة" style="width: 50px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                                </div>
                                            ...
                                            <div style="display: flex; align-items: center; justify-content: space-between; background: #f8fafc; padding: 6px 10px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                                ...
                                                <div style="display: flex; gap: 6px; align-items: center;">
                                                    <input type="number" name="configs[{{ $index }}][hard_count]" class="form-control" min="0" value="{{ $hardVal ?: '' }}" placeholder="العدد" style="width: 70px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                                    <span style="font-size: 10px; color: #94a3b8;">درجة:</span>
                                                    <input type="number" name="configs[{{ $index }}][hard_marks]" class="form-control" min="1" value="{{ $hardMarks }}" placeholder="الدرجة" style="width: 50px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                                </div>
                                    ...
                                    <div class="config-section">
                                        ...
                                        <div class="config-inputs-row" style="display: flex; flex-direction: column; gap: 8px;">
                                            ...
                                            <div style="display: flex; align-items: center; justify-content: space-between; background: #f8fafc; padding: 6px 10px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                                ...
                                                <input type="number" name="configs[{{ $index }}][mcq_count]" class="form-control" min="0" value="{{ $mcqVal ?: '' }}" placeholder="العدد" style="width: 70px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                            ...
                                            <div style="display: flex; align-items: center; justify-content: space-between; background: #f8fafc; padding: 6px 10px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                                ...
                                                <input type="number" name="configs[{{ $index }}][tf_count]" class="form-control" min="0" value="{{ $tfVal ?: '' }}" placeholder="العدد" style="width: 70px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                            ...
                                            <div style="display: flex; align-items: center; justify-content: space-between; background: #f8fafc; padding: 6px 10px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                                ...
                                                <input type="number" name="configs[{{ $index }}][matching_count]" class="form-control" min="0" value="{{ $matchingVal ?: '' }}" placeholder="العدد" style="width: 70px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
                                            ...
                                            <div style="display: flex; align-items: center; justify-content: space-between; background: #f8fafc; padding: 6px 10px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                                ...
                                                <input type="number" name="configs[{{ $index }}][essay_count]" class="form-control" min="0" value="{{ $essayVal ?: '' }}" placeholder="العدد" style="width: 70px; height: 30px; font-size: 11px; padding: 2px 6px;" oninput="validateConfigStats(this)">
...
<script>
    ...
    window.validateConfigStats = function(el) {
        ...
        if (!isNaN(totalMax)) {
            if (totalMax === 0) {
                ...
                qCountInput.style.borderColor = '#ef4444';
                qCountInput.style.boxShadow = '0 0 0 3px rgba(239, 68, 68, 0.15)';
                errorMsg += `<div class="text-danger" style="margin-bottom: 4px;"><i class="bi bi-exclamation-circle-fill"></i> العدد المطلوب (${qCount}) يتجاوز الحد الأقصى المتوفر (${totalMax}).</div>`;
                hasError = true;
            } else {
                qCountInput.style.borderColor = '#cbd5e1';
                qCountInput.style.boxShadow = 'none';
            }
        }

        // استخلاص مدخلات التوزيع المتقدم
        const getVal = (selector) => {
            const val = parseInt(card.querySelector(selector).value) || 0;
            return val;
        };
        ...
        // استخلاص الحد الأقصى المتوفر لكل نوع وصعوبة من شارات العرض الحالية
        const getMax = (selector) => {
            const el = card.querySelector(selector);
            return el ? (parseInt(el.dataset.max) || 0) : 0;
        };
        ...
        // دالة التحقق الفرعية للحقوق الفردية مع تحديث لون وتوفر الشارات الحية
        const validateField = (inputSelector, val, max, statsSelector) => {
            ...
            if (val > max) {
                ...
                statsEl.innerHTML = `<span class="text-danger font-weight-bold">تجاوز المتاح (${val}/${max})</span>`;
                hasError = true;
            } else if (val > 0) {
                inp.style.borderColor = 'var(--primary)';
                inp.style.boxShadow = 'none';
                statsEl.innerHTML = `<span class="text-primary font-weight-bold">محدد: ${val} / ${max}</span>`;
            } else {
                inp.style.borderColor = '#cbd5e1';
                inp.style.boxShadow = 'none';
                const colorMap = {
                    '.stats-easy': ['text-success'],
                    '.stats-medium': ['text-warning'],
                    '.stats-hard': ['text-danger'],
                    '.stats-mcq': ['text-primary'],
                    '.stats-tf': ['text-info'],
                    '.stats-matching': ['text-secondary'],
                    '.stats-essay': ['text-dark']
                };
    ...
    // تحميل إحصائيات الأسئلة وتفعيل التحقق عند تحميل الصفحة لأول مرة
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.subject-config-select').forEach(select => {
            if(select.value) {
                loadSubjectStats(select);
            }
        });
        validateGrades();
    });
